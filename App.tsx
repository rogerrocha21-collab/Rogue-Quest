...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      // Mantemos o estado carregado mas forçamos a tela inicial
      ...
      <div className="max-w-md w-full flex flex-col gap-4 animate-in fade-in duration-700">
        <header className="flex justify-between items-center border-b border-zinc-900 pb-2">
          ...
          <div className="text-right flex items-center gap-2">
            <button 
              onClick={handleShare}
              className="p-2 bg-zinc-900/50 hover:bg-zinc-800 rounded-lg border border-zinc-800 transition-colors text-zinc-400 hover:text-white"
              title="Compartilhar Bot"
            >
              <Icon.Share />
            </button>
            <button 
              onClick={() => setIsMuted(!isMuted)} 
              className="p-2 bg-zinc-900/50 hover:bg-zinc-800 rounded-lg border border-red-900/30 transition-colors text-red-500 hover:text-red-400"
            >
